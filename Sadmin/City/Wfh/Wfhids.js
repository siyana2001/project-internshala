...
import { BsGeoAlt, BsCalendar, BsBriefcase, BsFileText, BsBuilding, BsCashCoin, BsClock, BsPeople, BsCurrencyRupee, BsHouse, BsHouseDashFill } from 'react-icons/bs'; // Import icons from Bootstrap Icons
...
export const Wfhids = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`http://localhost:3009/getbyidw/${id}`);
                    ...
                    <Card style={{ width: 'fit-content', position: 'relative' }}>
                        <Card.Body style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                            <div style={{ color: '#6c757d' }}><h3>{value.companyname}</h3><br />
                                <h4> <BsHouseDashFill  />{value.location}</h4>
                            </div>
                            ...
                            <div style={{ marginBottom: '10px' }}>
                                <BsClock/> <span style={{ fontWeight: 'bold', fontStyle: 'italic' }}>Posted by</span> {value.posted} ago &nbsp;&nbsp;&nbsp;&nbsp; 
                           ...
                           <h5> <BsPeople /> <span style={{ fontWeight: 'bold', fontStyle: 'italic' }}></span> {value.applicants} applicants </h5>
                            ...
                            <Link to="/wfhs" className="btn btn-primary text-light" style={{ width: 'fit-content' }}>
                               &lt;- Back
                             </Link>
                             <button className="btn btn-danger text-light" onClick={() => { del(value.id) }}>Delete</button>
                             <Link to={`/updatewfh/${value.id}`}>
